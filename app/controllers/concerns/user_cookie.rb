module UserCookie
  def user_cookies
    if @current_user
      ...
      set_cookie(:user_name, @current_user.name)
      # set_cookie(:user_pic, @current_user.pic)

      user = Shop::User.try(:acquire, @current_user.id) if defined? Shop::User
      cart = JSON.parse(user.cart_data||'[]') rescue []
      session_cart = JSON.parse(session[:cart_data]||'[]') rescue []
      unless session_cart.blank?
        cart = (cart || []) + (session_cart || [])
        cart = cart.uniq { |unit| %w[product_id measure].map { |field| unit[field] }.join }.sort_by { |unit| cart.index(unit) }
        user.update_attributes :cart_data => cart.to_json
        session[:cart_data] = nil
      end
      set_cookie(:orders_count, cart.size > 0 ? cart.size : nil)
    else
