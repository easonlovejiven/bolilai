class Admin::Manage::EditorsController < Admin::Manage::ApplicationController
  ...
  def create
    ...
    account = Core::Account.acquire(@editor.id)
    core_user = account.user
    @editor.attributes={identifier: account.email,name: core_user.name,active: true}
    @manage_user.attributes={name: core_user.name,gender: core_user.sex,active: true}
  ...
  # def create
  #   @record = model.new
  #   @record.attributes = params.require(param_key).permit(*@record.manage_fields).merge(creator_id: @current_user.id)
  #   @success = database_transaction do
  #     @record.save!
  #     User.create! id: @record.id, email: "#{@record.prefix}@#{Rails.application.secrets[:domain]}", name: @record.name, crypted_password: '123456', password: '123456'
  #     Manage::User.create! id: @record.id, name: @record.name
  #   end
  #
  #   if @success
  ...
  # def update
  #   @editor = Manage::Editor.acquire(params[:id])
  #   @editor.attributes = (params[:manage_editor]||{}).slice(*@editor.class.manage_fields).merge(:user_id => @current_user.id)
  #   @editor.save
  #
  #   respond_to do |format|
  #     format.html { render :action => 'show' }
  ...
  def update
    ...
    @success = database_transaction do
      @record.save!
      User.f(id).update_attributes!(email: "#{@record.prefix}@qianfandu.com")
    end
