...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                actionUrl = editor.getOpt('qiniuUrl'),
            ...
            uploader.on('all', function (type, files) {
                switch (type) {
                    ...
                    case 'startUpload':
                        /* 添加额外的GET参数 */
                        uploader.option('server', editor.getOpt('qiniuUrl'));
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                var filename = "", token = "";
                if (type == 'guid') {
                    filename = (editor.getOpt('qiniuPath') || '') + guidKey() + "." + file.file.ext;
                } else if (type == 'date') {
                    //生成一个随机数目，防止批量上传的时候文件名同名出错
                    var randNumber = Math.floor(Math.random() * 10).toString()
                        + Math.floor(Math.random() * 20).toString();
                    filename = Date.parse(new Date()) + randNumber + "." + file.file.ext;
                } else {
                ...
                if (editor.getOpt('qiniuDomain')) {
                    data['domain'] = editor.getOpt('qiniuDomain');
                }
                data['key'] = filename;
                data['token'] = token;
            });
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    /** 保存到数据库 **/
                    if (json.key) {
                        var file_key = json.key;
                        if (editor.getOpt('qiniuPath'))
                            file_key = file_key.replace(editor.getOpt('qiniuPath'), "");
                        json['url'] = json.key;
                        json['key'] = file_key;
                        $.ajax({
                            url: editor.getOpt('imageActionName'),
                            type: 'POST',
                            async: false,
                            data: {
                                upload: {
            ...
            uploader.on('uploadError', function (file, code) {
                var $file = $('#' + file.id);
                $file.find('.error').text('失败,请检查图片。').show();
            });
    ...
    OnlineImage.prototype = {
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if (list[i] && list[i].file_key) {
                    //** 获取显示图片 **//
                    list[i]['url'] = editor.getOpt('qiniuPath') + list[i].file_key;
                    ...
                    img.width = 113;
                    //** 插入图片和显示图片 **//
                    img.setAttribute('src', urlPrefix + list[i].url + '-medium');
                    img.setAttribute('_src', urlPrefix + list[i].url + '-content');
